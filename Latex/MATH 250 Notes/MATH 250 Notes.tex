...
\end{framed}
\begin{df}[Power set]
	The \textbf{power set} of $A$, denoted $\pwer(A)$, is the set of all subset of $A$. Therefore, $|\pwer(A)|=2^n$ when $|A|=n.$
\end{df}
\begin{thm}
	\[x\in\pwer(A)\iff x\subseteq A\]	
\end{thm}
\begin{framed}
\noindent\texttt{Let $A, B$ be sets. Prove that $\pwer(A)\cup\pwer(B)\subseteq\pwer(A\cup B)$}
\begin{prf}
	Let $A$, $B$ be sets.\par Suppose $X\in\pwer(A)\cup\pwer(B)$ $\big[$Notation: Capital $X$ because $X$ is a set!$\big]$\par Then, by definition of union, $X\in\pwer(A)$ or $X\in\pwer(B).$\par WLOG, suppose $X\in\pwer(A).$ By definition of the power set, $X\subseteq A$.\par Since $A\subseteq A\cup B$, if $X\subseteq A$, then $X\subseteq A\cup B$.\par By definition of the power set, $X\in\pwer(A\cup B).$\par Hence, $\pwer(A)\cup\pwer(B)\subseteq\pwer(A\cup B).$
\end{prf}
\end{framed}
...
\begin{df}[Partition]
	Let $A$ be sets. A \textbf{partition} of $A$ is a subset of the power set of A, $\part\subseteq\pwer(A)$, satisfying the following three properties:
	\begin{enumerate}
