...
\end{eg}
\begin{lstlisting}[title=Bootstrap Sample]{language=r}
...
\begin{df}{Normal Distribution}
	The normal distribution is a function of two parameters: its mean and its standard deviation: \[p_{\mu,\sigma}(x)=\dfrac{1}{\sqrt{2\pi}\sigma}e^{-\dfrac{(x-\mu)^2}{2\sigma^2}}\]
\end{df}
...
\begin{thm}{Properties of Expectations}
	\begin{description}
		\item[Linearity of Expectation] Suppose $Y,Z$ are random variables and $a,b\in\R$. Then \[\E\qty(aY+bZ)=\E\qty(aY)+\E\qty(bZ)=a\E(Y)+b\E(Z).\] 
		\item[Multiplication Rules] Suppose $Y,Z$ are \emph{independent} random variables, then \[\E(Y\cdot Z)=\E(Y)\cdot\E(Z).\]
	\end{description}	
\end{thm}
\begin{rmk}
...
\end{rmk}
\begin{cor}{}
	If $Y,Z$ are independent, then $\Var(Y+Z)=\Var(Y)+\Var(Z).$	
\end{cor}
\begin{prf}
	\begin{align*}\Var(Y+Z)&=\E\qty[\qty(Y+Z)^2]-\E\qty(Y+Z)^2\\&=\E\qty(Y^2+Z^2+2YZ)-\qty[\E(Y)+\E(Z)]^2\\\text{Linearity}&=\E\qty(Y^2)+\E(Z^2)+2\E(YZ)-\E(Y)^2-\E(Z)^2-2\E(Y)\E(Z)\\\text{Independence}&=\qty(\E(Y^2)-\E(Y)^2)+\qty(\E(Z^2)-\E(Z)^2)+2\E(YZ)-2\E(YZ)\\&=\Var(Y)+\Var(Z).\end{align*}	
\end{prf}
\begin{thm}{Binomial Expection and Variance}
	If $Y\sim\text{Binomial}(n,p)$, then \[\E\qty(\bar{Y})=\E\qty[\dfrac{1}{n}\sum_{i=1}^n\qty(Y_i)]=p\] and \[\Var\qty(\bar{Y})=\dfrac{p(1-p)}{n}.\] The proof of these two quantities are omitted. 
\end{thm}
